...
public class SwitchLink {
  ...
  private ISwitchLinkable target(final Level world, final BlockPos source_pos) {
    if ((ModConfig.without_switch_linking()) || (!valid) || isTooFar(source_pos) || (!world.hasChunkAt(target_position)))
      return null;
    final BlockState target_state = world.getBlockState(target_position);
    final Block block = target_state.getBlock();
    if((!(block instanceof ISwitchLinkable)) || (!BuiltInRegistries.BLOCK.getKey(block).toString().equals(block_name)))
      return null;
